...
// Clasificacion de la imagen y su presentacion en mapa
function Clasificar(imagen, clasificador, etiqueta) {
  // resumen del clasificador CART
  print(etiqueta, clasificador);
  var imagen_clasificado = imagen.classify(clasificador)
  var nombre_capa = ee.String('Agua, Rural, Bosque, Urbano: ').cat(etiqueta)
  Map.addLayer(imagen_clasificado,
    {
        min: 0,
        max: 3,
        palette: ['blue','white','green','gray']
    }, nombre_capa.getInfo());
  return imagen_clasificado;
}

// Calculo y presentación de las estadisticas de precicion
function Precision(clasificado, control, etiqueta) {
  var muestreo_control = clasificado.sampleRegions({
      collection: control,
      scale: 20
  });
  var matriz_de_error = muestreo_control.errorMatrix('classification', 'clase', null);
  print(ee.String('Matriz de error ').cat(etiqueta), matriz_de_error);
  print(ee.String('Precisión general ').cat(etiqueta), matriz_de_error.accuracy());
  print(ee.String('Precisión de productor ').cat(etiqueta), matriz_de_error.producersAccuracy());
  print(ee.String('Precisión de usuario ').cat(etiqueta), matriz_de_error.consumersAccuracy());
  print(ee.String('Puntaje F1 ').cat(etiqueta), matriz_de_error.fscore());
  print(ee.String('Kappa de Cohen ').cat(etiqueta), matriz_de_error.kappa());
  return matriz_de_error;
}
...
// Preparar los poligonos de referencia
...
// Preparar subconjuntos de datos de entrenamiento (70%) y control (30%)
...
var entrenamiento_datos = imagen_Sentinel
                .sampleRegions({
...
print(entrenamiento_datos.limit(10))

// entrenamiento del clasificador CART
var clasificador_por_default = ee.Classifier.smileCart()
                 .train({
                 features: entrenamiento_datos,
                 classProperty: 'clase',
                 inputProperties: lista_capas
                });
...
var clasificador1 = ee.Classifier.smileCart({
                  maxNodes: 10
                }).train({
                 features: entrenamiento_datos,
                 classProperty: 'clase',
...
var imagen_clasificado_por_default = Clasificar(imagen_Sentinel,
                                                clasificador_por_default,
                                                'por default');
var imagen_clasificado_CART0 = Clasificar(imagen_Sentinel,
                                                clasificador0,
                                                'CART0');
var imagen_clasificado_CART1 = Clasificar(imagen_Sentinel,
                                                clasificador1,
                                                'CART1');    

var precision_por_default = Precision(imagen_clasificado_por_default,
                                      poligonos_control,
                                      'por default');
var precision_CART0 = Precision(imagen_clasificado_CART0,
                                      poligonos_control,
                                      'CART0');
var precision_CART1 = Precision(imagen_clasificado_CART1,
                                      poligonos_control,
                                      'CART1');
